...
# Try (in order): $WEBIDE_JDK, .../webstorm.jdk, .../jbr, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
JRE=""

# shellcheck disable=SC2154
if [ -n "$WEBIDE_JDK" ] && [ -x "$WEBIDE_JDK/bin/java" ]; then
  JRE="$WEBIDE_JDK"
fi

if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/WebStorm2022.3/webstorm.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/WebStorm2022.3/webstorm.jdk")
...
if [ -z "$JAVA_BIN" ] || [ ! -x "$JAVA_BIN" ]; then
  message "No JRE found. Please make sure \$WEBIDE_JDK, \$JDK_HOME, or \$JAVA_HOME point to valid JRE installation."
  exit 1
fi
...
if [ -n "$WEBIDE_VM_OPTIONS" ] && [ -r "$WEBIDE_VM_OPTIONS" ]; then
  ...
  if [ -r "${IDE_BIN_HOME}/webstorm64.vmoptions" ]; then
    VM_OPTIONS_FILE="${IDE_BIN_HOME}/webstorm64.vmoptions"
  else
    test "${OS_TYPE}" = "Darwin" && OS_SPECIFIC="mac" || OS_SPECIFIC="linux"
    if [ -r "${IDE_BIN_HOME}/${OS_SPECIFIC}/webstorm64.vmoptions" ]; then
      VM_OPTIONS_FILE="${IDE_BIN_HOME}/${OS_SPECIFIC}/webstorm64.vmoptions"
    fi
  fi
  ...
  elif [ -r "${CONFIG_HOME}/JetBrains/WebStorm2022.3/webstorm64.vmoptions" ]; then
    USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/WebStorm2022.3/webstorm64.vmoptions"
  fi
fi
...
CLASS_PATH="$CLASS_PATH:$IDE_HOME/lib/branding.jar"
